...
const ServicesEdit = () => {
    ...
    useEffect(() => {
        ...
        axios.post('http://localhost/projects/services_fetch.php', params)
            .then(function (response) {
                if (response.data) {
                    txtdetails.current.value = response.data['details'];
                }
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost/projects/services_edit.php', params)
            .then(function (response) {
                console.log(response);
                if (response.data['status'] === 'yes') {
                    alert("Data Successfully Edited.");
                    navigate('/servicesviewdata');
                } else {
                    alert("Data Edit Failed.");
                }
            })
    ...
    return (
        <>
            <h1>Service Data Edit Form</h1>
            <div className="form-container">
                <form method="POST" encType="multipart/form-data" onSubmit={handleSubmit}>
                    <input type="file" name="image" ref={txtimage} />
                    <input type="text" name="details" placeholder="Enter Service Details" ref={txtdetails} />
                    <input type="submit" name="submit" value="UPDATE" />
                </form>
            </div>
        </>
